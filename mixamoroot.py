...
def copyHips(root_bone_name="Root", hip_bone_name="mixamorig:Hips", name_prefix="mixamorig:"):
    ...
    bpy.ops.pose.select_all(action='DESELECT')
    bpy.context.object.pose.bones[hip_bone_name].select = True
    ...
    fcurves = [fcurve for fcurve in anim_utils.action_get_channelbag_for_slot(anim_data.action, anim_data.action_slot).fcurves if fcurve.data_path == 'pose.bones["{}"].location'.format(name_prefix + root_bone_name) and fcurve.array_index in range(3)]
    for i in fcurves:
        print(i.data_path)
    ...
    anim_data = bpy.context.object.animation_data
    action = anim_data.action if anim_data else None
    hips_fcurves = [hips_fcurve for hips_fcurve in anim_utils.action_get_channelbag_for_slot(anim_data.action, anim_data.action_slot).fcurves if hips_fcurve.data_path == 'pose.bones["{}"].location'.format(hip_bone_name) and hips_fcurve.array_index in range(3)]
